# --------------------------------------------------------------------------
#
# LIST OF REMAINING ISSUES:   NONE KNOWN AS OF 3/21/2016
#
#
# --------------------------------------------------------------------------
rm(list=ls())
...
#  in R 3.2.04;                               Last modified:  03/21/2016
#
...
getLast <- function(x){ t <- length(x); return(x[t]) }
outdir <- "Data/processed/"

# for 2015
#filename <- paste0('Data/', 'SCB2015_DAS.das')
# for 2011
filename <- paste0('Data/', 'SCB2011_DAS.das')

# creates part of output file name - remove directory names
DASfile <- paste("_", strsplit(getLast(unlist(strsplit(filename, '/'))), '.das'), '_', sep="")
#targetspchar <- c("DD","GG","LO","LB","PD")  # Provide these in upper case here
targetspchar <- c("CC")  # Provide these in upper case here
...
#-----------------------------------------------------
hdr2<-rep(targetspchar,each=2)
hdr1<-c("nSI","ANI")
sihdr<- paste(hdr1,hdr2,sep=".")
r<- length(targetspchar)*2
...
fn.getS <- function(DASLINES, slinenum, obsteam){
  ...
  while (substr(GSline,6,8)== '   ') {
    obok <- TRUE
    if (substr(GSline,4,4)!="C" &
        substr(GSline,4,4)!="?" &
        substr(GSline,46,49)!="xxxx") {
  ...
  Sitedata <- cbind(Sitedata0,
                    data.frame(spp=1, species=spp[1],
                               Sppgs=gs[1], Totgs=Totgs))
  if (nspp>1) {
    for (s in 2:nspp) {
      Specline <- cbind(Sitedata0,
                        data.frame(spp=s, species=spp[s],
                                   Sppgs=gs[s], Totgs=Totgs))
...
# FUNCTION to obtain turtle Sighting event data
# Returns object with multiple data values
#
fn.getT <- function(DASLINES, slinenum, obsteam){
  nspp <- 1    # always one for turtles
  Sline <- DASLINES[slinenum]
  sitenum <- substr(Sline,41,44)
  postime <- fn.getpostime(Sline)
  sitepos <- postime[1:2]
  ...
  #sppdata <- fn.trim(substr(Sline,60,80))       #Figure out how many species
  spp <- fn.trim(substr(Sline, 60, 64))
  size <- fn.trim(substr(Sline, 65, 69))
  tailvis <- fn.trim(substr(Sline, 70, 74))

  Specdata <- list(nspp=nspp, species=spp)    # make list of Spp info
  #  note that items set=NA here will be filled in in main program when segment is created
  Sitedata0 <- data.frame(segnum=NA, mlat=NA, mlon=NA, year=NA,
                          transectNum=NA, included=0, stime=sitetime,
                          snum=sitenum,  slat=sitepos[1], slon=sitepos[2],
  ...
  if (event1=='A') {
    ...
    for (i in 2:numlines) {
      if ((substr(effortseg[i],1,3)=='   '         # Check for and skip
        ...
        alldist[i] <- fn.grcirclkm(line1pos[1],line1pos[2],
                                   line2pos[1],line2pos[2])
        startpos <-0
      }
    }
    ...
    for (linenum in 2:length(effortseg))  {         #Run through data lines
      ...
      if (event == "t") {
        Sinfo <- fn.getT(effortseg, linenum, obs)
      }
      ...
      if (event=="S" || event == "t") {
        # Cumulative sighting tally
        sitecntr <- sitecntr + 1
        # Sighting should be scored on next segment, unless
        # it is right at the end of the segment (in which
        # case cumdist=0 and sum(alldist[1:linenum])>0
        # NOTE: the latter =0 if sighting is at R position,
        # in which case is should be scored on segnum + 1
        seg4sit <- segnum + 1

        if (round(cumdist,3)==0 &
            sum(alldist[1:linenum])>0) {
          seg4sit <- segnum
        }
        # [THIS FIXES ERROR FOUND BY PAUL FIEDLER 4/2015]
        spinfo <- Sinfo[1]
        # Record segment #
        Sinfo[[2]]$segnum  <- seg4sit
        if (Sinfo[[2]]$obtype[1]==1) {
          # dec angle checks later
          # For all species in sighting
          for (s in 1:spinfo[[1]]$nspp) {
            spchar  <- toupper(spinfo[[1]]$species[s])
            spchar <- fn.trim(spchar)
            # Is it a target species?
            species <- targetspchar[which(spchar==targetspchar)]
            # Check for pinniped, etc. codes to exclude.
            noncet <- which(exclcodes ==spinfo[[1]]$species[s])
            # the following if statement was removed for turtle sightings
            # 06-Jun-2016 TE
            # Only continue scoring if cetacean.
            #if (length(noncet)==0) {
            sp <- which(targetspchar==species)
            # If so, check declination angle
            if (length(sp)>0)  {
              da <- Sinfo[[2]]$DecAngle[1]
              da <- abs(da)
              # to see if within dec angle (NA's included by default)
              if (da >= TruncDecAngle |(is.na(da) & Incl.da.NA)) {
                # anis in appropriate col
                spcol<- ((sp-1)*2)+1
                allsites[seg4sit,spcol]<-allsites[seg4sit,spcol]+1
                if (event == "S") {
                  allsites[seg4sit,spcol+1] <- allsites[seg4sit, spcol+1] +
                    spinfo[[1]]$gs[s]}
...
write.csv(siteinfo,
          paste(outdir, "/", "SITEINFO", DASfile,
                seglen, "km", filedate, ".csv", sep=""),
...
write.csv(rand.out,
          paste(outdir, '/', "RandPicks",DASfile,
                seglen,"km",filedate,".csv",sep=""),
          quote = FALSE)
