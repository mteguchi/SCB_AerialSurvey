...
```{r}
layers <- ogrListLayers("Data/gpx files/2018-08-26_15-59-40 iSailGPS_Track.gpx")

# third layer is tracks
tracks_all <- readOGR(dsn = "Data/gpx files/2018-08-26_15-59-40 iSailGPS_Track.gpx",
                      layer = "track_points")
...
data_20180826 <- filter(data_coord_time_elev,
                        Date_local > as.POSIXct("2018-08-26 12:27:00", 
                                             format = "%Y-%m-%d %H:%M:%S")) %>%
...
```{r}
sightings.data <- read.csv(file = "data/Data_2018-08-26.csv",
...
for (k in 1:nrow(sightings.data)){
  ...
  loc <- filter(data_20180826, Date_local >= date.time - 1 & Date_local <= date.time + 1)
  if (nrow(loc) == 1){
...
data_20180826$event <- "*"
data_20180826$comment <- ""

sightings.data %>% select(event, Date_local, Latitude, Longitude, comment) -> sightings.data
data_20180826 %>% select(event, Date_local, Latitude, Longitude, comment) -> data_20180826

data_all <- rbind(sightings.data, data_20180826)
