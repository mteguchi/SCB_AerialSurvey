...
#sighted.locs <- subset(dat.all.df, nSI.CC > 0)
  ...
  # marine mammal survey sightings data:
  col_def.MMsurvey <- cols(Cruise = col_integer(),
                        Date = col_date('%d-%b-%y'),
                        Time = col_time('%H:%M:%S'),
                        Sight = col_character(),
                        Lat = col_double(),
  ...
  MMsurvey.data <- readr::read_csv(paste0(dirSelector()$Rdir,
                                             'Cc_SCB/data/TurtleSights_1.csv'),
                                   col_types = col_def.MMsurvey) %>%
    select(., Date, Lat, Lon, Species) %>%
    mutate(., Date = as.Date(Date)) %>%
    dplyr::rename(., Latitude = Lat, Longitude = Lon) %>%
    mutate(., Species = case_when(Species != 'CC' ~ 'Unid',
                                  Species == 'CC' ~ 'Caretta'))

  #all.data <- rbind(hotline.data, MMsurvey.data)

  SCB.data <- subset(MMsurvey.data, Longitude > -127.5 &
  ...
  for (k in 91:nrow(SCB.data)){
    ...
    file.name <- paste0('Data/sst_data/Cc_MMsurvey_sighting_', xlim[1], '_',
                        ylim[1], '_', tlim[[1]], '.nc')
  ...
  saveRDS(SCB.data.latlon,
          file = 'RDSfiles/SSTatMMsurveySightings.rds')
