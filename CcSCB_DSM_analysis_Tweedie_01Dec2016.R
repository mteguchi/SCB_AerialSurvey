...
M_xy_sst_chl <- dsm(N ~ s(x,y, k=10) + s(sst_mean, k=10) + s(logChl, k=10),
              ddf.obj = ds.model$ddf,
              segment.data = segment.data.2015,
              observation.data = observation.data)
...
M_sst_sst08_sst14_sst30 <- dsm(N ~ s(sst_mean, k=10) + s(sst_08_mean, k=10) +
                s(sst_14_mean, k=10) + s(sst_30_mean, k=10),
              ddf.obj = ds.model$ddf,
              segment.data = segment.data.2015,
              observation.data = observation.data)

# divergence warnings on this one
M_xy_chl_chl08_chl14_chl30 <- dsm(N ~ s(x,y, k=20) + s(logChl, k=10) +
                                    s(logChl_08, k=10) +
                                    s(logChl_14, k=10) + s(logChl_30, k=10),
              ddf.obj = ds.model$ddf,
              segment.data = segment.data.2015,
              observation.data = observation.data)

M_sst_sst08_sst14_sst30_depth <- dsm(N ~ s(sst_mean, k=10) + s(sst_08_mean, k=10) +
                s(sst_14_mean, k=10) + s(sst_30_mean, k=10) + s(depth_mean, k=10),
              ddf.obj = ds.model$ddf,
              segment.data = segment.data.2015,
              observation.data = observation.data)
