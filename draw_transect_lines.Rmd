...
E.end <- -117
W.end <- -118.3  # approx S tip of Catalina

NW.corner <- c(-118.3362, 33.4639)  # create a NW corner

line.length <- nm2km(20) * 1000 # length of each line from the coast line (m)
...
N.coords.df <- st_coordinates(N.coords) %>% data.frame()
...
while (lat > S.most.lat){
  # find new latitude - 90 degrees off from the first line
  tmp.pt <- data.frame(destPoint(E.coords[k, ], rhumb_bear-90, dlines))
  lat <- tmp.pt$lat
  new.line <-  st_as_sf(data.frame(X = c(W.end, E.end),
                                   Y = c(lat, lat)),
                        coords = c("X", "Y"),
                        crs = "+proj=longlat +datum=WGS84") %>%
    st_combine() %>%
  ...
  new.coords <- st_intersection(coast.line.linestring, new.line) %>%
    st_cast("POINT")
  
  new.coords.df <- st_coordinates(new.coords) %>% data.frame()
  new.coords.df[2,] <- destPoint(new.coords.df, rhumb_bear, line.length)
  
  coords.all[k1,] <- cbind(new.coords.df[1,], k)
  coords.all[k1+1,] <- cbind(new.coords.df[2,], k)
  k1 <- k1 + 2
  k <- k + 1
  W.coords[k, ] <- new.coords.df[2, ]
  E.coords[k, ] <- new.coords.df[1, ]
